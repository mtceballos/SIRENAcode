...
!!!!!!!!! W A R N I N G !!!!!!!!!!: There is an internal counter in astropy that saturates the INT range: when it reads
(n files * m columns) in a variable array and m*n > INT_MAX= 2147483647, it starts to do weird things
"""
...
def appendVarFits(fitsList, head, outfits):
    ...
    # Read headers from first file: HDU1 (and HDU0) will be (optionally) added to final file
    ...
    for icol in range(ncols):
        ...
        for infile in fitsList:
            ...
            for irow in range(nrows):
                ...
                COLdataList.append(dataForRow)
                if icol == 1:  #ADC
                    print(irow, ":", "data read by astropy in file",  infile, "around pulse=", dataForRow[1:4])
    ...
    for icol in range(ncols):
        # consider possibility of column being scaled => propagate keywords
    ...
    # NO IDEA WHY accesing data here IS IMPORTANT, BUT IF REMOVED --> WRONG VALUES IN FITS FILES
    #print("data written by astropy around pulse=", out[1].data.field(1)[0][1010:1030])
    try:
        print("data written by astropy around pulse in row=44652", out[1].data.field(1)[44651][1000:1004])
    except:
        print("keep on doing...")
