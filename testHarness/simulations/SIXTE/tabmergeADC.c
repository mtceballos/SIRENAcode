...
int main(int argc, char *argv[])
{
    ...
    // for ADC tessim simulated files
    double dtime;
    int adc[6096];
    int pixid;
    int phid;
    ...
    if ( fits_open_file(&infptr,  argv[1], READONLY,  &status) ||
        ...
        if (check && !status) 
        {
	        ...
	        // TIME
	        fits_read_col(infptr, TDOUBLE, 1, ii, 1,1, 0, &dtime, 0, &status);
                fits_write_col(outfptr, TDOUBLE, 1, jj, 1, 1, &dtime, &status);
                // ADC
                fits_read_col(infptr, TINT, 2, ii, 1, 6096, 0, adc, 0, &status);
                fits_write_col(outfptr, TINT, 2, jj, 1, 6096, adc, &status);
		// PIXID
	        fits_read_col(infptr, TINT, 3, ii, 1,1, 0, &pixid, 0, &status);
                fits_write_col(outfptr, TINT, 3, jj, 1, 1, &pixid, &status);
		// PH_ID
	        fits_read_col(infptr, TINT, 4, ii, 1,0, 0, &phid, 0, &status);
                fits_write_col(outfptr, TINT, 4, jj, 1, 0, &phid, &status);
