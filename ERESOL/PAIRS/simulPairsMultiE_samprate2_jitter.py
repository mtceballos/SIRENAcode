...
def simulPairs(pixName, monoEkeV1, monoEkeV2, acbias):
    ...
    for sep12 in sepsInt:
        ...
        root0 = "sep" + sepA + "sam_" + simTime + "s_" + monoEkeV1 + "keV_" + monoEkeV2 + "keV_samprate2_jitter"  # for piximpact
        root = "sep" + sepA + "sam_" + str(nSimPulses) + "p_" + monoEkeV1 + "keV_" + monoEkeV2 + "keV_samprate2_jitter"  # for fits

        pixFile = cwd + "/PIXIMPACT/" + root0 + "_trSz" + str(triggerSizeTC) + ".piximpact"
	#pixFile = PAIRSdir + "/PIXIMPACT/" + root0 + "_trSz" + str(triggerSizeTC) + ".piximpact"
        ...
        if not os.path.isfile(pixFile):
            comm = ("tesconstpileup PixImpList=" + pixFile + " XMLFile=" + XMLfile + 
                    ...
                    " pulseDistance=" + str(sep12) + " TriggerSize=" + str(triggerSizeTC) + " offset=-1 clobber=yes")
