...
drawLogPlotBox(xlimits=c(1E-4,1E3),ylimits=c(0,4.5), logxy="x",xlabel="Intensity (mCrab)",
...
for (ifi in 1:nfilters){
    for(idet in 1:nmodes){
        ...
        for (ifl in 1:nFluxes){
            ...
            lessThan05 <- sum(photonsInvolved<=1)
            if(lessThan05 > 0) {
                cat("Low energy (<=1keV) photons involved for (Flux,A1/AD,Filter)=(",
                    fluxes.mcrab[ifl],",",detectionModes[idet],",",legendTitles[ifi],"): ",
                    lessThan05, "/",length(photonsInvolved),"=",
                    sprintf("%.2f",lessThan05/length(photonsInvolved)),"\n")
