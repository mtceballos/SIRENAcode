...
def convertEnergies(inFile, outFile, coeffsFile, alias):
    """
    :param inFile: absolute path to file with reconstructed (non-calibrated) energies
    ...
    :param outFile: file with reconstructed/calibrated energies for the input pulses
    """
    ...
    hdr = f[0].header
    hdr['HISTORY'] = "Updated by convertEnergies.py to correct energies by gainscale"
    ftab = f[1].data
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Convert input energies to calibrated energies according to gainScale',
                                     prog='convertEnergies.py')

    parser.add_argument('--inFile', required=True,
                        help='Input FITS file (EVENTS) with reconstructed energies')
    parser.add_argument('--outFile', required=True,
                        help='Output FITS file (EVENTS) with calibrated energies')
    parser.add_argument('--coeffsFile', help='file with polynomial fit coefficients from R script polyfit2bias.R',
                        default='')
    parser.add_argument('--alias', help='String for reconstruction method in coefficient fuile '
                                        '(i.e. fixedlib6OF_OPTFILT4096', required=True)

    inargs = parser.parse_args()

    # print("array=",inargs.array)

    convertEnergies(inFile=inargs.inFile, outFile=inargs.outFile, coeffsFile=inargs.coeffsFile, alias=inargs.alias)
